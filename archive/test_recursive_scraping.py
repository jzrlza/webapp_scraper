...
def modifyRowRecursively(cur_elem_list, new_row) :
	if len(cur_elem_list) <= 0 :
		return
	for cur_elem in cur_elem_list :
		print(cur_elem.get_attribute('innerHTML').strip())
		if cur_elem.get_attribute('class') == "price" :
			new_row["price"] = numberCheckLambda(cur_elem.get_attribute('innerHTML').strip())
		modifyRowRecursively(cur_elem.find_elements(By.XPATH, '*'), new_row)
			...
			if not disabled_mode :
				...
				if clicked :
					...
					for i in range(len(init_web_contents)) :
						...
						modifyRowRecursively(web_content.find_elements(By.XPATH, '*'), new_row)
